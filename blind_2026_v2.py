"""2026年ブラインド検証 (新ルール: gap>=10%→馬連 / gap<10% and top4_cum>=60%→三連複 / else→見送り)"""
import sys, re, glob
...
print('=' * 74)
print('  2026年ブラインド検証 (新ルール)')
print('  gap>=10% → 馬連  /  gap<10% AND top4_cum>=60% → 三連複  /  else → 見送り')
print('=' * 74)
print(f'  {"月":<6} {"race_id":<14} {"レース名":<18} {"gap":>6} {"cum":>7} {"選択":<5} {"結果"} {"払戻":>8}')
print(f'  {"-"*74}')
...
for race_id in target_ids:
    ...
    pr = pay_rows.iloc[0]
    umaren_pays = parse_pair(pr.get('馬連払戻', ''))
    ...
    results.append({
        ...
        'gap': gap, 'top4_cum': top4_cum, 'bet': bet_type,
        'invest': invest, 'ret': ret, 'hit': hit,
    })
    print(f'  {month} {race_id:<14} {race_name[:16]:<18} {gap:>+5.1f}% {top4_cum:>6.1f}%  {bet_type:<5} {hit}  {ret:>7}円')

print(f'  {"-"*74}')
print(f'  突出外スキップ: {n_skip_not_totsu}件  見送り(top4_cum<60%): {n_skip_miosame}件')
